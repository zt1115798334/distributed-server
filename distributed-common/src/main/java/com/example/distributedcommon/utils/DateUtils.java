...
import com.example.distributedcommon.custom.SysConst.TimeType;
...
import static com.example.distributedcommon.custom.SysConst.TimeType.*;
...
public class DateUtils {
     ...
     * @return LocalDateTime
     */
    public static LocalDateTime currentDateTimeAddTime(long num, String timeUnit) {
        LocalDateTime currentDateTime = LocalDateTime.now();
        if (Objects.equal(timeUnit, SysConst.TimeUnits.DAYS.getType())) {
            return currentDateTime.plusDays(num);
        }
        if (Objects.equal(timeUnit, SysConst.TimeUnits.HOURS.getType())) {
            return currentDateTime.plusHours(num);
        }
        if (Objects.equal(timeUnit, SysConst.TimeUnits.MINUTES.getType())) {
            return currentDateTime.plusMinutes(num);
        }
    ...
    public static DateRange findDateTimeRange(String timeType, String startDateTime, String endDateTime) {
        if (Objects.equal(timeType, CUSTOM_TIME.getType())) {
            return DateRange.getIns(startDateTime, endDateTime);
        } else {
    ...
    public static DateRange findDateTimeRange(String timeType) {
        if (StringUtils.isNotEmpty(timeType)) {
            ...
            LocalDateTime endDateTime = currentDateTime;
            ...
            if (Objects.equal(timeType, TimeType.ALL.getType())) {
                LocalDateTime beforeMonthDateTime = currentDateTimeAddMonth(-3);
                startDateTime = dateTimeToFirstTime(beforeMonthDateTime);
            }
//        endDateTime = dateTimeToLastTime(currentDateTime);
            return DateRange.getIns(startDateTime, endDateTime);
        }
        return DateRange.getIns(StringUtils.EMPTY, StringUtils.EMPTY);
